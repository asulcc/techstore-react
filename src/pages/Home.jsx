...
export default function Home() {
  ...
  // useEffect: obtiene productos destacados desde JSON Server al montar la página.
  useEffect(() => {
    let activo = true;
    setCargando(true);
    getProductos()
    ...
    <div>
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 md:grid-cols-2 md:py-24">
          <div className="flex flex-col justify-center gap-5">
            ...
            <p className="max-w-md text-ink-muted">
              ...
              claros y disponibilidad real. Sin relleno, solo las specs que
              importan.
            </p>
          ...
          <div className="connector-frame relative flex items-center justify-center rounded-lg border border-graphite-border bg-graphite-surface/60 p-6">
            <img
              src="https://images.unsplash.com/photo-1587202372775-e229f172b9d7?w=700"
              alt="Estación de escritorio con periféricos gamer"
              className="h-full w-full rounded-md object-cover"
            />
          </div>
        </div>

        {/* Ticker de spec-strip: firma visual de la marca */}
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">
          {categorias.map((cat) => (
            <Link
              key={cat.nombre}
              to={cat.ruta}
              className="connector-frame rounded-lg border border-graphite-border bg-graphite-surface px-4 py-5 text-center transition-colors hover:border-copper/40"
            >
      ...
      <section className="mx-auto max-w-6xl px-4 py-6 pb-16 sm:px-6">
        <div className="mb-6 flex items-center justify-between">
          ...
          <Link to="/productos" className="font-mono text-xs uppercase tracking-wide text-copper hover:underline">
            Ver todos →
          </Link>
        </div>
        ...
        {!cargando && error && (
          <EmptyState
            ...
            message="Verifica que JSON Server esté corriendo en http://localhost:3000 (npm run server)."
          />
        )}

        {!cargando && !error && productos.length === 0 && (
          <EmptyState title="Sin productos" message="Aún no hay productos cargados en db.json." />
        )}
